 ...
 * Returns the rule set name if found, null otherwise
 */
async function detectExistingRuleSet() {
  try {
    const { stdout } = await execa('aws', [
      'ses', 'describe-active-receipt-rule-set',
      '--query', 'RuleSetMetadata.Name',
      '--output', 'text'
    ], { stdio: 'pipe' });
    
    const activeRuleSetName = stdout.trim();
...
async function initializeTerraform(config) {
  ...
  try {
    ...
    // Detect existing VCMail rule set BEFORE generating Terraform files
    // This allows us to set shared_rule_set_name in the config
    const existingRuleSet = await detectExistingRuleSet();
    if (existingRuleSet) {
      const computedConfig = getConfigWithDefaults(config);
      const projectRuleSetName = `${computedConfig.projectName}-incoming-email`;
      
      if (existingRuleSet === projectRuleSetName) {
        // Same project - will import normally
        console.log(chalk.cyan(`ℹ️  Found existing rule set for this project: ${existingRuleSet}`));
      } else {
        // Different VCMail project - will reuse existing rule set
        console.log(chalk.green(`✓ Found existing VCMail rule set: ${existingRuleSet}`));
        console.log(chalk.cyan(`   This project will add its rule to the shared rule set`));
        config.sharedRuleSetName = existingRuleSet;
      }
    }
...
async function importExistingSESResources(config) {
  try {
    ...
    const computedConfig = getConfigWithDefaults(config);
    const projectRuleSetName = `${computedConfig.projectName}-incoming-email`;
    const ruleSetName = config.sharedRuleSetName || projectRuleSetName;
    const ruleName = `${computedConfig.projectName}-email-rule`;
    
    try {
      // Check if we're using a shared rule set
      if (config.sharedRuleSetName && config.sharedRuleSetName !== projectRuleSetName) {
...
`;
  ...
  // Add shared_rule_set_name if detected
