 ...
 * @returns {Object} { ruleSetName: string|null, ruleExists: boolean, existingRules: Array }
 */
async function detectExistingRuleSet(config) {
  try {
    ...
    if (!result.Metadata || !result.Metadata.Name) {
      return { ruleSetName: null, ruleExists: false, existingRules: [] };
    }
    ...
    if (activeRuleSetName && activeRuleSetName !== 'None' && activeRuleSetName.endsWith('-incoming-email')) {
      // Check if our rule already exists in this rule set
