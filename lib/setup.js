...
async function validateAndCompleteConfig(existingConfig, skipPrompts = false) {
  ...
  if (questions.length === 0) {
    ...
    // Also remove Firebase fields - they should be loaded from SSM, not stored
    const firebaseFieldsToRemove = ['firebaseProjectId', 'firebaseDatabaseURL', 'firebaseApiKey', 'firebaseAppId', 'firebaseMessagingSenderId'];
    for (const field of [...fieldsToRemove, ...firebaseFieldsToRemove]) {
...
async function gatherConfiguration(existingConfig, skipPrompts = false) {
  ...
  return {
    ...
    // Note: Firebase config, CloudFront IDs, API endpoints, etc. are NOT stored
...
async function initializeTerraform(config) {
  ...
  try {
    ...
    // Parallelize resource discovery, then import sequentially
      ...
      if (importQueue.length > 0) {
        ...
        try {
          ...
          if (importError.message && importError.message.includes('already managed')) {
            ...
            if (process.env.DEBUG) {
              console.log(chalk.yellow(`   Could not import ${op.resource}: ${importError.message.split('\n')[0]}`));
            }
          }
        }
      } else {
...
async function discoverSESImports(config) {
  ...
  try {
    ...
    if ((config.sharedRuleSetName || config.activeRuleSetName) && ruleSetName !== projectRuleSetName) {
      // Using a shared rule set - only import the rule
      try {
        ...
        // Rule doesn't exist or error - skip
      }
    } else {
      ...
      try {
        ...
        if (stdout.trim() === 'True') {
          imports.push(
            {
              resource: 'aws_ses_receipt_rule_set.main[0]',
              importId: ruleSetName,
              message: `Attempting to import existing SES rule set: ${ruleSetName}`,
              successMessage: `Imported existing SES rule set: ${ruleSetName}`,
              alreadyManagedMessage: `SES rule set already in Terraform state`
            },
            {
              resource: 'aws_ses_active_receipt_rule_set.main[0]',
              importId: ruleSetName,
              message: `Attempting to import active SES rule set: ${ruleSetName}`,
              successMessage: `Imported active SES rule set: ${ruleSetName}`,
              alreadyManagedMessage: `Active SES rule set already in Terraform state`
            },
            {
              resource: 'aws_ses_receipt_rule.main',
              importId: `${ruleSetName}:${ruleName}`,
              message: `Attempting to import SES receipt rule: ${ruleName}`,
              successMessage: `Imported SES receipt rule: ${ruleName}`,
              alreadyManagedMessage: `SES receipt rule already in Terraform state`
            }
          );
        ...
        // Rule set doesn't exist or error - skip
      }
    }
  } catch (error) {
    // Non-critical - return empty array
...
async function loadFirebaseConfigFromSSM(config) {
  try {
    const computedConfig = getConfigWithDefaults(config);
    const ssm = new AWS.SSM({ region: computedConfig.awsRegion });
