...
async function gatherConfiguration(existingConfig, skipPrompts = false) {
  ...
  // Track which Firebase fields were successfully discovered
  const discoveredFields = {
    firebaseProjectId: !!(discoveredFirebaseConfig.firebaseProjectId),
    firebaseDatabaseURL: !!(discoveredFirebaseConfig.firebaseDatabaseURL),
    firebaseApiKey: !!(discoveredFirebaseConfig.firebaseApiKey),
    firebaseAppId: !!(discoveredFirebaseConfig.firebaseAppId),
    firebaseMessagingSenderId: !!(discoveredFirebaseConfig.firebaseMessagingSenderId)
  };
  
  // If any Firebase fields were discovered, inform the user
  const hasDiscoveredFields = Object.values(discoveredFields).some(v => v);
  if (hasDiscoveredFields && !skipPrompts) {
    console.log(chalk.cyan('Using discovered Firebase configuration values (no prompts needed):'));
    if (discoveredFields.firebaseProjectId) {
      console.log(chalk.green(`  ✓ firebaseProjectId: ${discoveredFirebaseConfig.firebaseProjectId}`));
    }
    if (discoveredFields.firebaseDatabaseURL) {
      console.log(chalk.green(`  ✓ firebaseDatabaseURL: ${discoveredFirebaseConfig.firebaseDatabaseURL}`));
    }
    if (discoveredFields.firebaseApiKey) {
      console.log(chalk.green(`  ✓ firebaseApiKey: ${discoveredFirebaseConfig.firebaseApiKey.substring(0, 10)}...`));
    }
    if (discoveredFields.firebaseAppId) {
      console.log(chalk.green(`  ✓ firebaseAppId: ${discoveredFirebaseConfig.firebaseAppId}`));
    }
    if (discoveredFields.firebaseMessagingSenderId) {
      console.log(chalk.green(`  ✓ firebaseMessagingSenderId: ${discoveredFirebaseConfig.firebaseMessagingSenderId}`));
    }
  ...
  const questions = [
    ...
    // Only include Firebase questions if they weren't discovered
    ...(discoveredFields.firebaseProjectId ? [] : [{
      type: 'input',
    ...
    ...(discoveredFields.firebaseDatabaseURL ? [] : [{
      type: 'input',
      name: 'firebaseDatabaseURL',
      message: 'Enter Firebase Realtime Database URL:',
      default: (answers) => configWithDefaults.firebaseDatabaseURL || existingConfig?.firebaseDatabaseURL || `https://${answers.firebaseProjectId || discoveredFirebaseConfig.firebaseProjectId || configWithDefaults.firebaseProjectId}.firebaseio.com`,
      validate: (input) => {
    ...
    ...(discoveredFields.firebaseApiKey ? [] : [{
      type: 'input',
      name: 'firebaseApiKey',
    ...
    ...(discoveredFields.firebaseAppId ? [] : [{
      type: 'input',
      name: 'firebaseAppId',
      message: 'Enter Firebase App ID (optional, defaults to project-id):',
      default: (answers) => configWithDefaults.firebaseAppId || existingConfig?.firebaseAppId || answers.firebaseProjectId || discoveredFirebaseConfig.firebaseProjectId || configWithDefaults.firebaseProjectId,
      required: false
    }]),
    ...(discoveredFields.firebaseMessagingSenderId ? [] : [{
      type: 'input',
  ...
  if (skipPrompts) {
    ...
    for (const question of questions) {
      const defaultValue = typeof question.default === 'function' 
        ? question.default({ ...existingConfig, ...discoveredFirebaseConfig, ...answers })
        : (question.default || existingConfig?.[question.name] || discoveredFirebaseConfig[question.name] || '');
      answers[question.name] = defaultValue;
      console.log(chalk.cyan(`  ${question.name}: ${defaultValue || '(empty)'}`));
    }
    // Also log discovered Firebase values being used
    if (hasDiscoveredFields) {
      console.log(chalk.cyan('\nUsing discovered Firebase configuration values:'));
      if (discoveredFields.firebaseProjectId) {
        console.log(chalk.green(`  ✓ firebaseProjectId: ${discoveredFirebaseConfig.firebaseProjectId}`));
      }
      if (discoveredFields.firebaseDatabaseURL) {
  ...
  // Merge discovered Firebase values into answers (discovered values take precedence over prompts)
  // This ensures that if discovery succeeded, we use those values even if user was prompted
  answers = {
    ...answers,
    ...discoveredFirebaseConfig
  };
