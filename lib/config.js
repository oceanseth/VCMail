 ...
 * Only domain, webmailDomain, and ssmPrefix are stored in config file
 * Everything else is derived from domain or loaded from SSM/Terraform outputs
 */
function getConfigWithDefaults(config = {}) {
  ...
  // Use provided ssmPrefix or derive from domain
  const ssmPrefix = config.ssmPrefix || deriveSSMPrefix(domain);
  ...
  return {
    ...
    // Firebase config should be loaded from SSM, not stored in config file
    // These are kept for backward compatibility and will be loaded from SSM during setup/deployment
    firebaseProjectId: config.firebaseProjectId || '',
    firebaseDatabaseURL: config.firebaseDatabaseURL || '',
    ...
    // CloudFront and API endpoint are detected from Terraform outputs, not stored
