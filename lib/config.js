 ...
 * Core fields (domain, webmailDomain, ssmPrefix, awsRegion, etc.) are stored in the config file.
 * Firebase project ID may be stored for Google APIs (Calendar) and client hints; full Firebase
 * secrets stay in SSM.
 */
function getConfigWithDefaults(config = {}) {
  ...
  return {
    ...
    // Firebase web client fields may be saved in vcmail.config.json after discovery; service account stays in SSM only.
    firebaseProjectId: config.firebaseProjectId || '',
    firebaseDatabaseURL: config.firebaseDatabaseURL || '',
